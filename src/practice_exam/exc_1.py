...
cns.print(
    "[bold cyan] Vamos a generar las gráficas del decaimiento con los datos que se inventó Claude."
)
...
gf.errorbar(
    ...
    yerr=sigma_n,
    fmt="o",
    label="Datos experimentales con barras de error",
    color="r",
    ecolor="black",
    capsize=5,
)
...
cns.print(
    Panel(
        f"Gráfica guardada como [bold]{filename}[/bold]",
        title="Éxto",
        subtitle="Generado por Rich",
        box=box.HEAVY,
    )
)
gf.show()
...
gf.title(
    "Decaimiento de particulas radiactivas con el cambio de parametros", fontsize=16
)
gf.xlabel("Tiempo (horas)", fontsize=14)
...
# Chi cuadrado
chi2, chi2_red = calculo_chis_cuadrado(
    ln_cuentas, m_linear, ttime, b_linear, sigma_ln_n
)
...
gf.plot(
    t_suave,
    ajuste,
    label=f"Ajuste exponencial N(t)={N_0:.2f}exp(-{lam:.2f}*t)",
    linewidth=2,
)
gf.xlabel("Tiempo (h)")
...
cns.print(
    Panel(
        f"Curva ajustada guardada como [bold]{filename}[/bold]",
        title="Éxito",
        box=box.HEAVY,
    )
)
