            ...
            for j in range(n):
                ...
                if sol[j, 2] <= 0:
                    ...
                    if abs(sol[j, 2]) <= tol / 2.0:
                        ...
                        n = len(sol) - 1
                        return (sol[n, 0] + sol[n - 1, 0]) / 2.0, groundtime
                        ...
                        r = array(
                            [sol[j - 1][0], sol[j - 1][1], sol[j - 1][2], sol[j - 1][3]]
                        )
                        t = linspace(t[j - 1], t[j], 50)
                        break
        s += 1
    print(
        "Distancia recorrida antes de llegar al suelo no encontrada\
...
def find_initial_velocity(distance, initial, v0, k):
    ...
    while i < it_max:
        ...
        if abs(fv) <= tol:
            return v0
        # Derivada numérica
        df = (f(v0 + dv / 2.0) - f(v0 - dv / 2.0)) / dv
        dv = -fv / df
        v0 += dv

        i += 1
    print(
        "Velocidad inicial no encontrada dentro de las iteraciones permitidas.\
          Regresamos 0."
    )
    return 0.0
...
for i, cd in enumerate(drag_coeffs):
    ...
    v_aux.append(
        v0_record
    )  # Agregar velocidad inicial encontrada a la lista --- nos serviran más adelante
    if v0_record == 0.0:
...
]  # Etiquetas para los regímenes
cd_labels = [
    "C_D = 0.0",
    "C_D = 0.5",
    "C_D = 0.75",
]  # Etiquetas para los coeficientes de arrastre
...
for i, cd in enumerate(drag_coeffs):  # Iterar sobre los coeficientes de arrastre
    k = rho * A * cd / 2.0
    distance, time = distance_reached(
        array([x0, y0]), v_aux[i], k
    ...
    sol, t = get_full_trajectory(
        array([x0, y0]), v_aux[i], k, time
    )  # Obtener trayectoria completa
    solutions.append(sol)  # Almacenar solución
    times_list.append(t)  # Almacenar tiempos
print("=" * 70)
print(
    "Calculo de las trayectorias con la velocidad inicial dada con C_D = 0.0: v0 =",
    v_aux[0],
)
print("\nCalculando trayectorias...")
...
for i, (sol, t, cd, label, cd_label, color, distance) in enumerate(
    ...
    fig.suptitle(
        f"{label}\n{cd_label} | v₀ = {v_aux[i]:.2f} m/s", fontsize=15, fontweight="bold"
    )
