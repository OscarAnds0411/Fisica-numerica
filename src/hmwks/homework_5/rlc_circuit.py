...
sigma_vo = sigma_a1(
    S_xx(uncer, ttime), delta(S(uncer), S_xx(uncer, ttime), S_x(uncer, ttime))
)
...
if chi2_reducido < 0.5:
    interpretacion = (
        "[yellow] χ²_red < 0.5: Posible sobreestimación de errores[/yellow]"
    )
elif 0.5 <= chi2_reducido <= 2.0:
    interpretacion = "[green] 0.5 ≤ χ²_red ≤ 2.0: Ajuste excelente[/green]"
elif 2.0 < chi2_reducido <= 5.0:
    interpretacion = (
        "[yellow] 2.0 < χ²_red ≤ 5.0: Ajuste aceptable, revisar errores[/yellow]"
    )
else:
...
if 0.5 <= chi2_reducido <= 2.0:
    console.print(
        "  [green] SÍ. El modelo exponencial describe bien los datos.[/green]"
    )
    console.print(
        "  [green]  Los residuos son consistentes con las incertidumbres.[/green]"
    )
else:
...
plt.plot(
    ...
    color="b",
    linewidth=2.5,
    label=f"Ajuste: V₀={v_0_linear:.3f} V, τ={tau_linear:.3f} ms⁻¹",
    alpha=0.8,
)
plt.plot(
    ttime,
