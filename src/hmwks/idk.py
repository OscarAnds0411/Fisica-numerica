"""
3. Ajuste de la fÛrmula de resonancia de Breit-Wigner.
(a) Para la teorÌa indica que la fÛrmula de Breit-Wigner debe ajustar
...
Su problema consiste en determinar los valores para los par·met-
ros Er, fr y

. Se sugiere renombrar los p·rametros haciendo
a1 = fr; ; a2 = ER; a3 =
2
4
; x = E;
...
2 + a3
;
y encontrar los p·rametros a partir de minimizar 
2
, encuentre

estas ecuaciones.
(b) Las ecuaciones que obtuvo en el inciso anterior NO son lineales,
elabore un programa que utilice el mÈtodo de Newton-Raphson
multidimensional para la b ̇squeda de las raÌces.
"""
...
def NewtonRapshon(a_0, da, x, y, sc, epsilon):
    ...
    for i in range(Nmax):               # -Evitamos ciclos infinitos.
        ...
        a -= np.linalg.dot(IDiff_F, F)        # -Actualizamos a = [a_1, a_2, a_3]
...
a_0 = np.array([45000.0, 70.0, 700.0])             # -Semilla.
...
print('f_r = ', a[0], ', E_r = ', a[1], ' Gamma = ', 2.0*np.sqrt(a[2]))
...
plt.xlabel('x'); plt.ylabel('f(x)'); plt.title('Ajuste g(x) vs x')
plt.plot(x, y, 'o', lw = 2, color = 'r')
