...
console.rule(
    "[bold red]AJUSTE DEL ESPECTRO DE CUERPO NEGRO - RADIACIÓN CÓSMICA DE FONDO (COBE)"
)
...
"""
panel = Panel(
    const, title="[bold]Constantes a usar[/bold]", border_style="green", box=box.DOUBLE
)
console.print(panel)
...
# mostramos los datos
table = Table(
    title="[bold yellow]Datos del COBE cargados[/bold yellow]", box=box.ROUNDED
)
...
table = Table(
    title="[bold cyan] Datos recuperados con panda de COBE.txt[/bold cyan]",
    box=box.SQUARE,
)
...
plt.title("Datos del COBE - Radiación Cósmica de Fondo", fontsize=15, fontweight="bold")
...
try:
    ...
    console.print(
        f"[bold blue]  Error relativo = {100*abs(T_fit - T_cmb_accepted)/T_cmb_accepted:.2f}%"
    )
    ...
    ax1.plot(
        ...
        color="blue",
        linewidth=3,
        label=f"Ajuste Planck: T = {T_fit:.3f} K",
        alpha=0.8,
    )
    ...
    ax1.axvline(
        ...
        linestyle="--",
        linewidth=2,
        label=f"ν_max teórico = {nu_max_teorico:.2f} cm⁻¹",
        alpha=0.6,
    )
    ...
    ax1.set_title("Ajuste del Espectro de Cuerpo Negro", fontsize=15, fontweight="bold")
    ...
    plt.suptitle(
        "Análisis Completo del Espectro de Cuerpo Negro - CMB",
        fontsize=18,
        fontweight="bold",
        y=0.995,
    )
...
(b) Temperatura de la radiación cósmica de fondo:
    ...
    * Evidencia fundamental del modelo cosmológico estándar
    """
    panel = Panel(
        sms, title="[bold]Finalmente:\n[/bold]", border_style="green", box=box.DOUBLE
    )
    console.print(panel)
